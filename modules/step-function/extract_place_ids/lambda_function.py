...
def handler(event, context):
    ...
    restaurant_bucket_name = os.environ.get("RESTAURANT_BUCKET", "naver-map-restaurant")
    
    # 파일 키 생성 (query 기반) - URL encoding 적용
    file_key = f"{unquote_plus(query)}.json"
    
    try:
        # S3에서 파일 읽기
        response = s3_client.get_object(Bucket=restaurant_bucket_name, Key=file_key)
